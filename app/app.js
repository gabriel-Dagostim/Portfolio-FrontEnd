...
function clickAct() {
    ...
    if (isChecked) {
        ...
        document.getElementById('p-desc-project-02').innerHTML = "O Projeto de Festa de 15 Anos foi criado com o objetivo de celebrar um marco especial utilizando uma combinação de novas tecnologias. Pela primeira vez, foram utilizados WordPress e Elementor para criar um site único e visualmente atraente, personalizado para a celebração da debutante. Além disso, o jQuery foi incorporado para aprimorar os elementos interativos, e o Tailwind CSS foi usado para estilizar o site com recursos de design moderno e responsivo. Uma adição notável ao site é uma loja integrada onde os convidados podem comprar presentes para a celebração. Esse recurso não apenas adiciona conveniência, mas também aprimora a experiência geral do evento. O projeto serve como um exemplo prático de como aplicar habilidades de desenvolvimento web para criar uma experiência online memorável para um evento marcante. A natureza aberta e personalizável do site permite ajustes e personalizações futuras, tornando-o uma ferramenta versátil para projetos semelhantes.";
